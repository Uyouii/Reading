...
本文分析 CrewAI 如何通过角色分工、任务约束、流程编排和经验复用完成复杂任务。重点追踪模型与工具如何协作、状态如何持久化与恢复，以及反馈如何影响后续任务。
...
| 层次 | 管理什么 | 提供的能力 |
|---|---|---|
| Flow | 触发、分支、汇合、状态、暂停恢复 | 组织调查、审批与执行阶段 |
| Crew / Process | 任务集合、上下文、顺序或层级协作 | 阶段内任务分工与协作 |
...
flowchart TD
    ...
    PREP --> MODEL["请求模型"]
    MODEL --> CALL{"模型发起工具调用？"}
    CALL -->|是| TOOL["解析参数并执行工具<br/>可能委派其他 Agent 或加载技能"]
    TOOL --> OBS["把工具结果加入消息"]
    OBS --> LIMIT{"达到循环边界？"}
    LIMIT -->|否| MODEL
...
| `max_iter=25` | 达上限进入收束回答 | 限制单任务循环，不约束跨任务的累计运行预算 |
...
## 9. 端到端示例：售后异常调查中的协作与恢复

以下是应用设计示意，并非已验证的仓库业务系统。以“调查反复投诉的订单，形成方案，经审批后更新工单”为例，观察并行调查、任务协作、审批恢复与经验保存如何衔接。

| 阶段 | 编排与动作 | 保存 / 验证 |
|---|---|---|
| 登记 | 外部服务创建请求 ID | state 存订单、工单、阶段、操作 ID；查已有结果 |
| 并行调查 | 两个 listener 调只读 Agent，核验订单与投诉，按需读技能 | 分支写互不冲突的字段 |
| 形成方案 | `and_` 汇合后启动 Crew；调查 Agent 提供证据，汇总 Agent 生成方案 | 固定依赖用顺序任务，动态补充用 manager |
