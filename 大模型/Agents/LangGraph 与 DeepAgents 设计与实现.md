...
本文围绕工作流编排、状态恢复与能力组合，分析 LangGraph 如何保存和推进工作流，DeepAgents 如何在其上组织模型、工具、技能、记忆和子任务。重点追踪一次任务从接收、执行、暂停、恢复到验收的过程，并区分当前任务状态与跨任务经验。
...
## 9. 端到端示例：问题工单中的状态流转与经验复用

以下为应用设计示意，**不是仓库自带工单系统或实测记录**。以问题调查、方案核验和交付为例，观察上下文准备、子任务委派、暂停恢复与跨任务记忆如何协作。

```mermaid
flowchart TD
    A["接收工单：明确目标、约束与验收项"] --> B["读取相关记忆与技能"]
...
| 稳定业务约定、已核查经验 | 按用户或业务域组织的 Store | 以后任务加载或检索 |
